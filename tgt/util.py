...
def revert_to_backslash_trigraphs(tier, latin_only = False):
    '''Convert Unicode characters back to backslash trigraphs.

    This function restores backslash sequences automatically converted
    by Praat to Unicode characters. If latin_only is False (the
    default) only Latin characters are replaced. Otherwise,
    replacement is applied to all affected characters.
    '''

    trans_latin = str.maketrans({
        'ä': r'\a\"',
        'ë': r'\e\"',
        'ï': r'\i\"',
        'ö': r'\o\"',
    ...
    tier_trigraphs = copy.deepcopy(tier)
    for intr in tier_trigraphs:
        intr.text = intr.text.translate(trans_latin)
        if not latin_only:
            intr.text= intr.text.translate(trans_non_latin)
    return tier_trigraphs
