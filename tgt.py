...
def read_short_textgrid(filename, encoding='utf-8'):
    ...
    if stg[2].startswith('xmin'):
        raise Exception('TextGrid is not in the short format: {0}'.format(filename))
    ...
    while index < len(stg):
        ...
        if stg[index] == '"IntervalTier"':
            tg.add_tier(read_short_interval_tier(stg[index:index+5+num_obj*3]))
            index += 5 + (num_obj * 3)
        elif stg[index] == '"TextTier"':
            tg.add_tier(read_short_point_tier(stg[index:index+5+num_obj*2]))
            index += 5 + (num_obj * 2)
        else:
...
def read_short_interval_tier(stg_extract):
...
def read_short_point_tier(stg_extract):
...
def get_long_attr_val(x):
    """Extracts the attribute value from a long TextGrid line."""
    return x.split(' = ')[1]

def read_long_textgrid(filename, encoding='utf-8'):
    ...
    if stg[1] != 'Object class = "TextGrid"':
        raise Exception(filename)
    if not stg[2].startswith('xmin'):
        raise Exception('TextGrid is not in the long format: {0}'.format(filename))
    read_start_time = float(get_long_attr_val(stg[2]))
    read_end_time = float(get_long_attr_val(stg[3]))
    if stg[4].split()[1] != '<exists>':
        raise Exception(filename)
    read_no_of_tiers = get_long_attr_val(stg[5])
    index = 7
    while index < len(stg):
        num_obj  = int(get_long_attr_val(stg[index+5]))
        if get_long_attr_val(stg[index+1]) == '"IntervalTier"':
            tg.add_tier(read_long_interval_tier(stg[index:index+6+num_obj*4]))
            index += 6 + (num_obj * 4)
        elif get_long_attr_val(stg[index+1]) == '"TextTier"':
            tg.add_tier(read_long_point_tier(stg[index:index+6+num_obj*3]))
            index += 6 + (num_obj * 3)
        else:
            raise Exception('Unknown tier type: {0}'.format(stg[index]))
    return tg

def read_long_interval_tier(stg_extract):
    '''Reads and returns an IntervalTier from a long TextGrid.'''
    name = get_long_attr_val(stg_extract[2])[1:-1] # name w/o quotes
    start_time = float(get_long_attr_val(stg_extract[3]))
    end_time = float(get_long_attr_val(stg_extract[4]))
    it = IntervalTier(start_time, end_time, name)
    i = 7
    while i < len(stg_extract):
        it.add_interval(Interval(float(get_long_attr_val(stg_extract[i])), # left bound
            float(get_long_attr_val(stg_extract[i+1])),                    # right bound 
            get_long_attr_val(stg_extract[i+2])[1:-1]))                    # text w/o quotes
        i += 4
    return it

def read_long_point_tier(stg_extract):
    '''Reads and returns a PointTier (called TextTier) from a long TextGrid.'''    
    name = get_long_attr_val(stg_extract[1])[1:-1] # name w/o quotes
    start_time = float(get_long_attr_val(stg_extract[3]))
    end_time = float(get_long_attr_val(stg_extract[4]))
    pt = PointTier(start_time, end_time, name)
    i = 7
    while i < len(stg_extract):
        pt.add_point(Point(float(get_long_attr_val(stg_extract[i])), # time
            get_long_attr_val(stg_extract[i+1])[1:-1]))              # text w/o quotes
        i += 3
    return pt
